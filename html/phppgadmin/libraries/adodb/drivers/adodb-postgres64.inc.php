...
WHERE relkind in ('r','v') AND (c.relname='%s' or c.relname = lower('%s'))
 and c.relnamespace=n.oid and n.nspname='%s' 
    ...
    // function ADODB_postgres64() // GZI : /!\ c'est également de le nom de cette la classe !!!
	...
	union 
        ...
        if ($mask) {
            ...
            if ($info ['version'] >= 7.3) $this->metaTablesSQL = "
select tablename,'T' from pg_tables where tablename like $mask and schemaname not in ( 'pg_catalog','information_schema')  
 union 
select viewname,'V' from pg_views where viewname like $mask and schemaname  not in ( 'pg_catalog','information_schema')  ";
            else $this->metaTablesSQL = "
...
select viewname,'V' from pg_views where viewname like $mask";
    ...
    function MetaColumns($table, $normalize = true) {
        ...
        while ( ! $rs->EOF ) {
            ...
            if (is_array ( $keys )) {
                foreach ( $keys as $key ) {
                    if ($fld->name == $key ['column_name'] and $key ['primary_key'] == 't') $fld->primary_key = true;
                    if ($fld->name == $key ['column_name'] and $key ['unique_key'] == 't') $fld->unique = true; // What name is more compatible?
                }
            }
...
WHERE (c2.relname=\'%s\' or c2.relname=lower(\'%s\'))';
        ...
        while ( $row = $rs->FetchRow () ) {
            ...
            $indexes [$row [0]] = array ('unique' => ($row [1] == 't'),'columns' => $columns );
        }
        return $indexes;
    }
    ...
    function _query($sql, $inputarr = false) {
        ...
        if ($inputarr) {
            ...
            if ($rez) $rez = pg_exec ( $this->_connectionID, $exsql );
        }
        else {
            // adodb_backtrace();
...
class ADORecordSet_postgres64 extends ADORecordSet {
    ...
    function _initrs() {
        ...
        if (empty ( $this->connection->noBlobs )) for($i = 0, $max = $this->_numOfFields; $i < $max; $i ++) {
    ...
    function _fetch() {
        if ($this->_currentRow >= $this->_numOfRows && $this->_numOfRows >= 0) return false;
    ...
    function MetaType($t, $len = - 1, $fieldobj = false) {
        ...
        switch (strtoupper ( $t )) {
            ...
            case 'INT4' :
            case 'INT2' :
                if (isset ( $fieldobj ) && empty ( $fieldobj->primary_key ) && (! $this->connection->uniqueIisR || empty ( $fieldobj->unique ))) return 'I';
