...
class ADODB_postgres7 extends ADODB_postgres64 {
    ...
    // function ADODB_postgres7() // GZI
    function __construct() {
        // $this->ADODB_postgres64(); // GZI
    ...
    function SelectLimit($sql, $nrows = - 1, $offset = - 1, $inputarr = false, $secs2cache = 0) {
        ...
        if ($secs2cache) $rs = $this->CacheExecute ( $secs2cache, $sql . "$limitStr$offsetStr", $inputarr );
        else $rs = $this->Execute ( $sql . "$limitStr$offsetStr", $inputarr );
    ...
    function _old_MetaForeignKeys($table, $owner = false, $upper = false) {
        ...
        foreach ( $arr as $v ) {
            ...
            for($i = 4; $i < $size; $i ++) {
                if ($upper) $a [strtoupper ( $data [2] )] [] = strtoupper ( $data [$i] . '=' . $data [++ $i] );
                else $a [$data [2]] [] = $data [$i] . '=' . $data [++ $i];
            }
        }
        return $a;
    }
    ...
    function _updatefields() {
        ...
        foreach ( $this->fields as $k => $v ) {
            if (is_integer ( $k )) $arr [$k] = $v;
                ...
                if ($lowercase) $arr [strtolower ( $k )] = $v;
                else $arr [strtoupper ( $k )] = $v;
            }
        }
        $this->fields = $arr;
    }
