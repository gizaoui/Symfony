...
if (! defined ( '_ADODB_LAYER' )) {
    ...
    GLOBAL $ADODB_vers, // database version
    $ADODB_COUNTRECS, // count number of records returned - slows down query
    $ADODB_CACHE_DIR, // directory to cache recordsets
    $ADODB_CACHE, $ADODB_CACHE_CLASS, $ADODB_EXTENSION, // ADODB extension installed
    $ADODB_COMPAT_FETCH, // If $ADODB_COUNTRECS and this is true, $rs->fields is available on EOF
    $ADODB_FETCH_MODE, // DEFAULT, NUM, ASSOC or BOTH. Default follows native driver default...
    $ADODB_GETONE_EOF, $ADODB_QUOTE_FIELDNAMES; // Allows you to force quotes (backticks) around field names in queries generated by getinsertsql and getupdatesql.
    ...
    function ADODB_Setup() {
        ...
        $ADODB_COUNTRECS, // count number of records returned - slows down query
        $ADODB_CACHE_DIR, // directory to cache recordsets
        $ADODB_FETCH_MODE, $ADODB_CACHE, $ADODB_CACHE_CLASS, $ADODB_FORCE_TYPE, $ADODB_GETONE_EOF, $ADODB_QUOTE_FIELDNAMES;
    ...
    class ADOConnection {
        ...
        function Connect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "", $forceNew = false) {
            ...
            if ($fn = $this->raiseErrorFn) $fn ( $this->databaseType, 'CONNECT', $this->ErrorNo (), $err, $this->host, $this->database, $this );
        ...
        function PConnect($argHostname = "", $argUsername = "", $argPassword = "", $argDatabaseName = "") {
            if (defined ( 'ADODB_NEVER_PERSIST' )) return $this->Connect ( $argHostname, $argUsername, $argPassword, $argDatabaseName );
        ...
        /**
         * Some databases, eg.
         * mssql require a different function for preparing
         ...
         *            of table to lock
         * @param $where where
         *            clause to use, eg: "WHERE row=12". If left empty, will escalate to table lock
         */
         ...
         * Used together with CompleteTrans().
         * Advantages include:
         *
        ...
        function CompleteTrans($autoComplete = true) {
            ...
            if ($this->_transOK && $autoComplete) {
                ...
                else if ($this->debug) ADOConnection::outp ( "Smart Commit occurred" );
            }
            else {
                $this->_transOK = false;
                $this->RollbackTrans ();
         ...
         */
        function FailTrans() {
            if ($this->debug) if ($this->transOff == 0) {
                ...
                if (! is_array ( $sql ) && ! $this->_bindInputArray) {
                    ...
                    foreach ( $inputarr as $arr ) {
                        ...
                        foreach ( $arr as $v ) {
                            ...
                            $sql .= $this->qstr ( $v );
                            else if ($typ == 'double') $sql .= str_replace ( ',', '.', $v ); // locales fix so 1.1 does not get converted to 1,1
                            else if ($typ == 'boolean') $sql .= $v ? $this->true : $this->false;
                            else if ($typ == 'object') {
                                if (method_exists ( $v, '__toString' )) $sql .= $this->qstr ( $v->__toString () );
                                else $sql .= $this->qstr ( ( string ) $v );
                            }
                            else if ($v === null) $sql .= 'NULL';
                            else $sql .= $v;
                            $i += 1;
                        ...
                        else if ($i != sizeof ( $sqlarr )) $this->outp_throw ( "Input array does not match ?: " . htmlspecialchars ( $sql ), 'Execute' );
                    ...
                    if ($array_2d) {
                        if (is_string ( $sql )) $stmt = $this->Prepare ( $sql );
                        else $stmt = $sql;
         ...
         *
         * @param $table string
         *            name of the table, not needed by all databases (eg. mysql), default ''
         * @param $column string
         *            name of the column, not needed by all databases (eg. mysql), default ''
        ...
        function MetaPrimaryKeys($table, $owner = false) {
            ...
            if ($objs) {
                foreach ( $objs as $v ) {
                    if (! empty ( $v->primary_key )) $p [] = $v->name;
                }
            }
            if (sizeof ( $p )) return $p;
            if (function_exists ( 'ADODB_VIEW_PRIMARYKEYS' )) return ADODB_VIEW_PRIMARYKEYS ( $this->databaseType, $this->database, $table, $owner );
            return false;
        }
        ...
        function SelectLimit($sql, $nrows = - 1, $offset = - 1, $inputarr = false, $secs2cache = 0) {
            if ($this->hasTop && $nrows > 0) {
                ...
                if ($offset <= 0) {
                    ...
                    if ($isaccess) {
                        $sql = preg_replace ( '/(^\s*select\s+(distinctrow|distinct)?)/i', '\\1 ' . $this->hasTop . ' ' . (( integer ) $nrows) . ' ', $sql );
                        ...
                        $sql = preg_replace ( '/(^\s*select\s)/i', '\\1 ' . $this->hasTop . ' ' . (( integer ) $nrows) . ' ', $sql );
                    }
                }
                else {
                    $nn = $nrows + $offset;
        ...
        function CacheExecute($secs2cache, $sql = false, $inputarr = false) {
            ...
            if (! $rs) {
                ...
                else if (! $this->memCache) $ADODB_CACHE->flushcache ( $md5file );
            }
            else {
                $this->_errorMsg = '';
                $this->_errorCode = 0;
         ...
         * @return void
         */
        function GetActiveRecordsClass($class, $table, $whereOrderBy = false, $bindarr = false, $primkeyArr = false, $extra = array (), $relations = array ()) {
         ...
         *
         * @param $ok set
         *            to false to rollback transaction, true to commit
         *            
         * @return true/false.
         */
         ...
         *
         * @param
         *            ttype can either be 'VIEW' or 'TABLE' or false.
         ...
         * @param $table table
         *            name to query
         * @param $normalize makes
         *            table name case-insensitive (required by some databases)
        ...
        function MetaColumnNames($table, $numIndexes = false, $useattnum = false /* only for postgres */) 
	{
        ...
        function DBTimeStamp($ts, $isfld = false) {
            ...
            if (! is_string ( $ts ) || (is_numeric ( $ts ) && strlen ( $ts ) < 14)) return adodb_date ( $this->fmtTimeStamp, $ts );
        ...
        static function UnixDate($v) {
            ...
            if (! preg_match ( "|^([0-9]{4})[-/\.]?([0-9]{1,2})[-/\.]?([0-9]{1,2})|", ($v), $rr )) return false;
         ...
         *
         * @param $v is
         *            a timestamp string in YYYY-MM-DD HH-NN-SS format
         *            
         * @return date in unix timestamp format, or 0 if before TIMESTAMP_FIRST_YEAR, or false if invalid date format
         */
         ...
         * @param
         *            [magic_quotes] if $s is GET/POST var, set to get_magic_quotes_gpc().
         ...
         * $nrows rows per page.
         * It also saves two boolean values saying if the given page is the first
    ...
    /**
     * Internal placeholder for record objects.
    ...
    class ADORecordSet implements IteratorAggregate {
        ...
        var $adodbFetchMode; // GZI : Declaration absente
         ...
         * @param
         *            [selectAttr] additional attributes to defined for SELECT tag.
         ...
         *            
         * @return HTML changes by glen.davies@cce.ac.nz to support multiple hilited items
         */
        ...
        function GetMenu3($name, $defstr = '', $blank1stItem = true, $multiple = false, $size = 0, $selectAttr = '') {
            global $ADODB_INCLUDED_LIB;
            if (empty ( $ADODB_INCLUDED_LIB )) include (ADODB_DIR . '/adodb-lib.inc.php');
            return _adodb_getmenu_gp ( $this, $name, $defstr, $blank1stItem, $multiple, $size, $selectAttr, false );
        }
         ...
         *
         * @param
         *            [force_array] has only meaning if we have 2 data columns. If false, a 1 dimensional
         *            array is returned, otherwise a 2 dimensional array is returned. If this sounds confusing,
         *            read the source.
         *            
         * @param
         *            [first2cols] means if there are more than 2 cols, ignore the remaining cols and
        ...
        function Move($rowNumber = 0) {
            ...
            if ($rowNumber >= $this->_numOfRows) if ($this->_numOfRows != - 1) $rowNumber = $this->_numOfRows - 2;
         ...
         *
         * @param $isupper to
         *            set the object property names to uppercase
         *            
         * @return the object with the properties set to the fields of the current row
         */
         ...
         * This is used for formatting. This is because
         ...
         * @param
         *            len is the maximum length of that field. This is because we treat character
         *            fields bigger than a certain size as a 'B' (blob).
         * @param
         *            fieldobj is the field object returned by the database driver. Can hold
    ...
    class ADORecordSet_array extends ADORecordSet {
         ...
         * @param
         *            typearr holds an array of types. These are the same types
         *            used in MetaTypes (C,B,L,I,N).
         * @param
         *            [colnames] array of column names. If set, then the first row of
     ...
     *
     * @param
     *            [db] is the database Connection object to create. If undefined,
    ...
    function ADONewConnection($db = '') {
        ...
        if (($at = strpos ( $db, '://' )) !== FALSE) {
            ...
            if (strncmp ( $origdsn, 'pdo', 3 ) == 0) {
                ...
                if (sizeof ( $sch ) > 1) {
                    ...
                    if ($sch [1] == 'sqlite') $dsna ['host'] = rawurlencode ( $sch [1] . ':' . rawurldecode ( $dsna ['host'] ) );
                    else $dsna ['host'] = rawurlencode ( $sch [1] . ':host=' . rawurldecode ( $dsna ['host'] ) );
                    $dsna ['scheme'] = 'pdo';
                }
            }
        ...
        if ($obj) {
            ...
            if (isset ( $dsna )) {
                ...
                if (empty ( $persist )) $ok = $obj->Connect ( $dsna ['host'], $dsna ['user'], $dsna ['pass'], $dsna ['path'] );
                else if (empty ( $nconnect )) $ok = $obj->PConnect ( $dsna ['host'], $dsna ['user'], $dsna ['pass'], $dsna ['path'] );
                else $ok = $obj->NConnect ( $dsna ['host'], $dsna ['user'], $dsna ['pass'], $dsna ['path'] );
    ...
    function NewDataDictionary(&$conn, $drivername = false) {
        ...
        if (! empty ( $conn->_connectionID )) $dict->serverInfo = $conn->ServerInfo ();
