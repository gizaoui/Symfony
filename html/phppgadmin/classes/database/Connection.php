...
class Connection {
     ...
     * Will actually make a database connection.
     *
     * @param $fetchMode Defaults
     *            to associative. Override for different behaviour
     */
    function __construct($host, $port, $sslmode, $user, $password, $database, $fetchMode = ADODB_FETCH_ASSOC) {
        ...
        // Ignore host if null
        if ($host === null || $host == '') if ($port !== null && $port != '') $pghost = ':' . $port;
        else $pghost = '';
     ...
     * sets the platform.
     *
     * @param (return-by-ref) $description
     *            A description of the database and version
    ...
    function getDriver(&$description) {
        ...
        if (! isset ( $version )) {
            ...
            if (preg_match ( '/ mingw /i', $field )) $this->platform = 'MINGW';
