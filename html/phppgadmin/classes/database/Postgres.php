...
class Postgres extends ADODB_base {
    ...
    // Function properties
    var $funcprops = array (
            array ('','VOLATILE','IMMUTABLE','STABLE' ),
    ...
    var $predefined_size_types = array (
            'abstime',
            'aclitem',
            'bigserial',
            'boolean',
    ...
    var $triggerEvents = array ('INSERT','UPDATE','DELETE','INSERT OR UPDATE','INSERT OR DELETE','DELETE OR UPDATE','INSERT OR DELETE OR UPDATE' );
    ...
    /**
     * Cleans (escapes) an object name (eg.
     * table, field)
     *
     * @param $str The
     *            string to clean, by reference
     ...
     * @param $value The
     *            value of the field. Note this could be 'numeric(7,2)' sort of thing...
     * @param $type The
     *            database type of the field
     * @param $extras An
     *            array of attributes name as key and attributes' values as value
     */
     ...
     *            or EXPRESSION
     * @param $value The
     *            actual value entered in the field. Can be NULL
        ...
        switch ($type) {
            ...
            default :
                ...
                if (strpos ( $type, 'time' ) === 0) {
                    ...
                    elseif ($format == 'EXPRESSION') return $value;
                    else {
                        $this->clean ( $value );
                        return "'{$value}'";
                    }
    ...
    /**
     * Formats a type correctly for display.
     * Postgres 7.0 had no 'format_type'
     * built-in function, and hence we need to do it manually.
     *
     * @param $typname The
     *            name of the type
     * @param $typmod The
     *            contents of the typmod field
     */
    function formatType($typname, $typmod) {
        ...
        elseif ($typname == 'varchar') {
            ...
            if ($typmod != - 1) $temp .= "(" . ($typmod - $varhdrsz) . ")";
        }
     ...
     *
     * @param $currentdatabase database
     *            name that should be on top of the resultset
     *            
     ...
     *            the name of the database to get the comment for
     ...
     *
     * @param string $database
     *            the name of the database to get the owner for
     ...
     *
     * @param string $dbName
     *            database to change ownership of
     ...
     *            Find only prepared transactions executed in a specific database
     * @return A recordset
     */
    function getPreparedXacts($database = null) {
        if ($database === null) $sql = "SELECT * FROM pg_prepared_xacts";
        else {
            $this->clean ( $database );
     ...
     *            search term
     * @param $filter The
     *            object type to restrict to ('' means no restriction)
     * @return A recordset
     */
     ...
     * Will also set Class variable.
     *
     * @param $schema The
     *            the name of the schema to work in
     * @return 0 success
     */
     ...
     *
     * @param $paths An
     *            array of schemas in required search order
     * @return 0 success
     ...
     *
     * @param $schemaname The
     *            name of the schema to create
     * @param $authorization (optional)
     *            The username to create the schema for.
     * @param $comment (optional)
     *            If omitted, defaults to nothing
     * @return 0 success
     */
     ...
     * @param $comment The
     *            new comment for this schema
     * @param $owner The
     *            new owner for this schema
     * @return 0 success
     */
     ...
     *            name of the schema to drop
     * @param $cascade True
     *            to cascade drop, false to restrict
     * @return 0 success
     */
     ...
     *
     * @param $all True
     *            to fetch all tables, false for just in current schema
     ...
     *
     * @param $table The
     *            table to find the children for
     * @return A recordset
     */
     ...
     *            table to define
     * @param $clean True
     *            to issue drop command, false otherwise
    ...
    function getTableDefPrefix($table, $clean = false) {
        ...
        while ( ! $atts->EOF ) {
            ...
            if ($this->phpBool ( $atts->fields ['attisserial'] ) && ($atts->fields ['type'] == 'integer' || $atts->fields ['type'] == 'bigint')) {
                if ($atts->fields ['type'] == 'integer') $sql .= " SERIAL";
                else $sql .= " BIGSERIAL";
            }
            else {
                ...
                if ($this->phpBool ( $atts->fields ['attnotnull'] )) $sql .= " NOT NULL";
                // Add default if necessary
                if ($atts->fields ['adsrc'] !== null) $sql .= " DEFAULT {$atts->fields['adsrc']}";
            }
        ...
        while ( ! $cons->EOF ) {
            ...
            if ($cons->fields ['consrc'] !== null) $sql .= $cons->fields ['consrc'];
            else {
        ...
        // Handle WITHOUT OIDS
        if ($this->hasObjectID ( $table )) $sql .= " WITH OIDS";
        else $sql .= " WITHOUT OIDS";
     ...
     * @param $array An
     *            array of '' or '[]' for each type if it's an array or not
     * @param $length An
     ...
     * @param $withoutoids True
     *            if WITHOUT OIDS, false otherwise
     * @param $colcomment An
     *            array of comments
     * @param $comment Table
     *            comment
     * @param $tablespace The
     *            tablespace name ('' means none/default)
     * @param $uniquekey An
     *            Array indicating the fields that are unique (those indexes that are set)
     * @param $primarykey An
     *            Array indicating the field used for the primarykey (those indexes that are set)
     * @return 0 success
    ...
    function createTable($name, $fields, $field, $type, $array, $length, $notnull, $default, $withoutoids, $colcomment, $tblcomment, $tablespace, $uniquekey, $primarykey) {
        ...
        // WITHOUT OIDS
        if ($withoutoids) $sql .= ' WITHOUT OIDS';
        else $sql .= ' WITH OIDS';
     ...
     *            name of the table
     * @param $like an
     *            array giving the schema and the name of the table from which attribs are copying from:
     *            array(
     *            'table' => table name,
     *            'schema' => the schema name,
     *            )
     * @param $defaults if
     *            true, copy the default values as well
     * @param $constraints if
     *            true, copy the constraints as well (CHECK on table & attr)
     * @param $tablespace The
     *            tablespace name ('' means none/default)
     */
     ...
     *
     * @param $tblrs The
     *            table RecordSet returned by getTable()
     * @param $name The
     *            new table's name
     * @return 0 success
     */
    function alterTableName($tblrs, $name = null) {
        ...
        if (! empty ( $name ) && ($name != $tblrs->fields ['relname'])) {
            ...
            $status = $this->execute ( $sql );
            if ($status == 0) $tblrs->fields ['relname'] = $name;
            else return $status;
        }
        return 0;
    }
     ...
     * @param $owner The
     *            new owner for the table
     * @param $schema The
     *            new schema for the table
     * @param $comment The
     *            comment on the table
     * @param $tablespace The
     *            new tablespace for the table ('' means leave as is)
     * @return 0 success
     * @return -3 rename error
     * @return -4 comment error
    ...
    function alterTable($table, $name, $owner, $schema, $comment, $tablespace) {
        ...
        if ($data->recordCount () != 1) return - 2;
     ...
     *
     * @param $table The
     *            table to get attributes for
     * @param $atts An
     *            array of attribute numbers
     ...
     *            type of the column
     * @param $array True
     *            if array type, false otherwise
     * @param $notnull True
     *            if NOT NULL, false otherwise
     * @param $default The
     *            default for the column. '' for none.
     * @param $length The
     *            optional size of the column (ie. 30 for varchar(30))
     * @return 0 success
     */
    function addColumn($table, $column, $type, $array, $length, $notnull, $default, $comment) {
        ...
        if ($length == '') $sql = "ALTER TABLE \"{$f_schema}\".\"{$table}\" ADD COLUMN \"{$column}\" {$type}";
        else {
            switch ($type) {
                // Have to account for weird placing of length for with/without
                // time zone types
     ...
     *
     * @param $table The
     *            table in which the column resides
     * @param $column The
     *            column to alter
     * @param $name The
     ...
     * @param $oldnotnull (boolean)
     *            True if column is already not null, false otherwise
     * @param $default The
     *            new default for the column
     * @param $olddefault The
     *            old default for the column
     * @param $type The
     ...
     *
     * @param $table The
     *            table containing the column to be renamed
     * @param $column The
     ...
     *            column to alter
     * @param $state True
     *            to set null, false to set not null
     * @return 0 success
     */
     ...
     *
     * @param $table The
     *            table from which to drop a column
     * @param $column The
    ...
    /**
     * Sets up the data object for a dump.
     * eg. Starts the appropriate
    ...
    /**
     * Returns a recordset of all columns in a relation.
     ...
     *
     * @param $table if
     *            given, return autovacuum info for the given table or return all information for all tables
     *            
     * @return A recordset
     */
     ...
     *
     * @param $table The
     *            table for which to retrieve the identifier
     ...
     * @param $fields Array
     *            of given field in values
     * @param $values Array
     *            of new values for the row
     * @param $nulls An
     *            array mapping column => something if it is to be null
     * @param $format An
     *            array of the data type (VALUE or EXPRESSION)
     * @param $types An
     *            array of field types
     * @return 0 success
        ...
        if (! is_array ( $fields ) || ! is_array ( $values ) || ! is_array ( $nulls ) || ! is_array ( $format ) || ! is_array ( $types ) || (count ( $fields ) != count ( $values ))) {
            ...
            if (count ( $values ) > 0) {
                ...
                foreach ( $values as $i => $value ) {
                    ...
                    // Handle NULL values
                    if (isset ( $nulls [$i] )) $sql .= ',NULL';
                    else $sql .= ',' . $this->formatValue ( $types [$i], $format [$i], $value );
                }
     ...
     * @param $vars An
     *            array mapping new values for the row
     * @param $nulls An
     *            array mapping column => something if it is to be null
     * @param $format An
     *            array of the data type (VALUE or EXPRESSION)
     ...
     *            array of field types
     * @param $keyarr An
     *            array mapping column => value to update
     * @return 0 success
     * @return -1 invalid parameters
     */
    function editRow($table, $vars, $nulls, $format, $types, $keyarr) {
        if (! is_array ( $vars ) || ! is_array ( $nulls ) || ! is_array ( $format ) || ! is_array ( $types )) return - 1;
        else {
            $f_schema = $this->_schema;
            $this->fieldClean ( $f_schema );
            $this->fieldClean ( $table );
     ...
     * @param $key An
     *            array mapping column => value to delete
     * @return 0 success
     */
     ...
     * @param $maxvalue The
     *            max value
     * @param $startvalue The
     *            starting value
     * @param $cachevalue The
     *            cache value
     * @param $cycledvalue True
     *            if cycled, false otherwise
     * @return 0 success
     */
     ...
     *
     * @param $seqrs The
     *            sequence RecordSet returned by getSequence()
     * @param $name The
    ...
    function alterSequenceName($seqrs, $name) {
        ...
        if (! empty ( $name ) && ($seqrs->fields ['seqname'] != $name)) {
            ...
            $status = $this->execute ( $sql );
            if ($status == 0) $seqrs->fields ['seqname'] = $name;
            else return $status;
        }
        return 0;
    }
     ...
     *            sequence RecordSet returned by getSequence()
     * @param $increment The
     *            sequence incremental value
     * @param $minvalue The
     *            sequence minimum value
     * @param $maxvalue The
     *            sequence maximum value
     * @param $restartvalue The
     *            sequence current value
     * @param $cachevalue The
     *            sequence cache value
     * @param $cycledvalue Sequence
     *            can cycle ?
     * @param $startvalue The
     *            sequence start value when issuing a restart
     * @return 0 success
     */
     ...
     * @param $definition The
     *            definition for the new view
     * @param $replace True
     *            to replace the view, false otherwise
     * @return 0 success
     */
     ...
     *
     * @param $vwrs The
     *            view recordSet returned by getView()
     * @param $name The
     *            new view's name
     * @return 0 success
     */
    function alterViewName($vwrs, $name) {
        ...
        if (! empty ( $name ) && ($name != $vwrs->fields ['relname'])) {
            ...
            $status = $this->execute ( $sql );
            if ($status == 0) $vwrs->fields ['relname'] = $name;
            else return $status;
        }
        return 0;
    }
    ...
    protected function _alterView($vwrs, $name, $owner, $schema, $comment) {
        ...
        // Comment
        if ($this->setComment ( 'VIEW', $vwrs->fields ['relname'], '', $comment ) != 0) return - 4;
     ...
     *
     * @param $table The
     *            name of a table whose indexes to retrieve
     * @param $unique Only
     *            get unique/pk indexes
     * @return A recordset
     */
    ...
    function alreadyClustered($table) {
        ...
        if ($v->recordCount () == 0) return false;
     ...
     *            index name
     * @param $table The
     *            table on which to add the index
     * @param $columns An
     *            array of columns that form the index
     ...
     *            predicate ('' for none)
     * @param $tablespace The
     *            tablespaces ('' means none/default)
     * @return 0 success
     */
     ...
     *            or 'TABLE' or 'INDEX'
     * @param $name The
     *            name of the specific database, table, or index to be reindexed
     * @param $force If
     *            true, recreates indexes forcedly in PostgreSQL 7.0-7.1, forces rebuild of system indexes in 7.2-7.3, ignored in >=7.4
     */
     ...
     *
     * @param $table the
     *            table where we are looking for fk
     * @return a recordset
     */
     ...
     *
     * @param $table The
     *            table to which to add the primery key
     * @param $fields (array)
     *            An array of fields over which to add the primary key
     * @param $name (optional)
     *            The name to give the key, otherwise default name is assigned
     * @param $tablespace (optional)
     *            The tablespace for the schema, '' indicates default.
     * @return 0 success
     ...
     *
     * @param $table The
     *            table to which to add the unique key
     * @param $fields (array)
     *            An array of fields over which to add the unique key
     * @param $name (optional)
     *            The name to give the key, otherwise default name is assigned
     * @param $tablespace (optional)
     *            The tablespace for the schema, '' indicates default.
     ...
     *
     * @param $table The
     *            table to which to add the check
     * @param $definition The
     *            definition of the check
     * @param $name (optional)
     *            The name to give the check, otherwise default name is assigned
     * @return 0 success
     */
     ...
     *
     * @param $table The
     *            table from which to drop the check
     * @param $name The
     *            name of the check to be dropped
     * @return 0 success
     ...
     *
     * @param $targschema The
     *            schema that houses the target table to which to add the foreign key
     * @param $targtable The
     *            table to which to add the foreign key
     * @param $target The
     *            table that contains the target columns
     * @param $sfields (array)
     *            An array of source fields over which to add the foreign key
     * @param $tfields (array)
     *            An array of target fields over which to add the foreign key
     * @param $upd_action The
     *            action for updates (eg. RESTRICT)
     * @param $del_action The
     *            action for deletes (eg. RESTRICT)
     * @param $match The
     *            match type (eg. MATCH FULL)
     * @param $deferrable The
     *            deferrability (eg. NOT DEFERRABLE)
     * @param $initially The
     *            initial deferrability (eg. INITIALLY IMMEDIATE)
     * @param $name (optional)
     *            The name to give the key, otherwise default name is assigned
     * @return 0 success
     * @return -1 no fields given
    ...
    function addForeignKey($table, $targschema, $targtable, $sfields, $tfields, $upd_action, $del_action, $match, $deferrable, $initially, $name = '') {
        if (! is_array ( $sfields ) || sizeof ( $sfields ) == 0 || ! is_array ( $tfields ) || sizeof ( $tfields ) == 0) return - 1;
        $f_schema = $this->_schema;
        $this->fieldClean ( $f_schema );
        $this->fieldClean ( $table );
     ...
     *            constraint to drop
     * @param $relation The
     *            relation from which to drop
     * @param $type The
     *            type of constraint (c, f, u or p)
     * @param $cascade True
     *            to cascade drop, false to restrict
     * @return 0 success
     */
     ...
     *
     * @param $tables multi
     *            dimensional assoc array that holds schema and table name
     ...
     *
     * @param $domain The
     *            name of the domain whose constraints to fetch
     * @return A recordset
     */
     ...
     *            type length
     * @param $array True
     *            for array type, false otherwise
     * @param $notnull True
     *            for NOT NULL, false otherwise
     * @param $default Default
     *            value for domain
     * @param $check A
     *            CHECK constraint if there is one
     * @return 0 success
     */
     ...
     * @param $domain The
     *            domain to alter
     * @param $domdefault The
     *            domain default
     * @param $domnotnull True
     *            for NOT NULL, false otherwise
     * @param $domowner The
     *            domain owner
     * @return 0 success
    ...
    function alterDomain($domain, $domdefault, $domnotnull, $domowner) {
        ...
        // Default
        if ($domdefault == '') $sql = "ALTER DOMAIN \"{$f_schema}\".\"{$domain}\" DROP DEFAULT";
        else $sql = "ALTER DOMAIN \"{$f_schema}\".\"{$domain}\" SET DEFAULT {$domdefault}";
        ...
        // NOT NULL
        if ($domnotnull) $sql = "ALTER DOMAIN \"{$f_schema}\".\"{$domain}\" SET NOT NULL";
        else $sql = "ALTER DOMAIN \"{$f_schema}\".\"{$domain}\" DROP NOT NULL";
     ...
     *
     * @param $domain The
     *            domain to which to add the check
     * @param $definition The
     *            definition of the check
     * @param $name (optional)
     *            The name to give the check, otherwise default name is assigned
     ...
     *
     * @param $domain The
     *            domain from which to remove the constraint
     * @param $constraint The
     *            constraint to remove
     * @param $cascade True
     *            to cascade, false otherwise
     * @return 0 success
     */
     ...
     *
     * @param $all If
     *            true, will find all available functions, if false just those in search path
     * @param $type If
     *            not null, will find all functions with return value = type
     *            
     ...
     *
     * @param $f The
     *            array of data for the function
    ...
    function getFunctionProperties($f) {
        ...
        // Volatility
        if ($f ['provolatile'] == 'v') $temp [] = 'VOLATILE';
        elseif ($f ['provolatile'] == 'i') $temp [] = 'IMMUTABLE';
        elseif ($f ['provolatile'] == 's') $temp [] = 'STABLE';
        else return - 1;
        
        // Null handling
        $f ['proisstrict'] = $this->phpBool ( $f ['proisstrict'] );
        if ($f ['proisstrict']) $temp [] = 'RETURNS NULL ON NULL INPUT';
        else $temp [] = 'CALLED ON NULL INPUT';
        
        // Security
        $f ['prosecdef'] = $this->phpBool ( $f ['prosecdef'] );
        if ($f ['prosecdef']) $temp [] = 'SECURITY DEFINER';
        else $temp [] = 'SECURITY INVOKER';
     ...
     *            return type
     * @param $definition The
     *            definition for the new function
     * @param $language The
     *            language the function is written for
     * @param $flags An
     *            array of optional flags
     * @param $setof True
     *            if returns a set, false otherwise
     * @param $comment The
     ...
     *            array of optional flags
     * @param $setof True
     *            if it returns a set, false otherwise
     * @param $rows number
     *            of rows planner should estimate will be returned
     * @param $cost cost
     *            the planner should use in the function execution step
     * @param $comment Comment
     *            for the function
     * @param $replace (optional)
     *            True if OR REPLACE, false for normal
     * @return 0 success
     ...
     *
     * @param $all If
     *            true, will find all available types, if false just those in search path
     * @param $tabletypes If
     *            true, will include table types
     * @param $domains If
     *            true, will include domains
     * @return A recordet
     */
    function getTypes($all = false, $tabletypes = false, $domains = false) {
        ...
        // Create type filter
        $tqry = "'c'";
        if ($tabletypes) $tqry .= ", 'r', 'v'";
        
        // Create domain filter
        if (! $domains) $where .= " AND t.typtype != 'd'";
     ...
     * @param $table Table
     *            on which to create the rule
     * @param $where When
     *            to execute the rule, '' indicates always
     * @param $instead True
     *            if an INSTEAD rule, false otherwise
     * @param $type NOTHING
     *            for a do nothing rule, SOMETHING to use given action
     * @param $action The
     *            action to take
     * @return 0 success
     ...
     *            action to take
     * @param $replace (optional)
     *            True to replace existing rule, false otherwise
     * @return 0 success
     * @return -1 invalid event
     */
     ...
     *
     * @param $table The
     *            name of a table whose triggers to retrieve
     * @param $trigger The
     *            name of the trigger to retrieve
     * @return A recordset
     */
     ...
     *
     * @param $trigger An
     *            array containing fields from the trigger table
    ...
    function getTriggerDef($trigger) {
        ...
        if ($trigger ['tgisconstraint']) $tgdef = 'CREATE CONSTRAINT TRIGGER ';
        else $tgdef = 'CREATE TRIGGER ';
        
        $tgdef .= "\"{$trigger['tgname']}\" ";
        
        // Trigger type
        $findx = 0;
        if (($trigger ['tgtype'] & TRIGGER_TYPE_BEFORE) == TRIGGER_TYPE_BEFORE) $tgdef .= 'BEFORE';
        else $tgdef .= 'AFTER';
        ...
        if ($trigger ['tgisconstraint']) {
            ...
            if (! $trigger ['tgdeferrable']) $tgdef .= 'NOT ';
            $tgdef .= 'DEFERRABLE INITIALLY ';
            if ($trigger ['tginitdeferred']) $tgdef .= 'DEFERRED ';
            else $tgdef .= 'IMMEDIATE ';
        }
        
        // Row or statement
        if ($trigger ['tgtype'] & TRIGGER_TYPE_ROW == TRIGGER_TYPE_ROW) $tgdef .= 'FOR EACH ROW ';
        else $tgdef .= 'FOR EACH STATEMENT ';
        ...
        for($findx = 0; $findx < $trigger ['tgnargs']; $findx ++) {
            ...
            $tgdef .= $param;
            if ($findx < ($trigger ['tgnargs'] - 1)) $tgdef .= ', ';
        }
     ...
     *
     * @param $table The
     *            name of the table containing the trigger
     * @param $trigger The
     ...
     * @param $table The
     *            table in which to enable the trigger
     * @return 0 success
     */
     ...
     *
     * @param $tgname The
     *            name of the trigger to disable
     * @param $table The
     *            table in which to disable the trigger
     * @return 0 success
     */
     ...
     *            The name of the FTS configuration to create
     * @param string $parser
     *            The parser to be used in new FTS configuration
     * @param string $locale
     *            Locale of the FTS configuration
     * @param string $template
     *            The existing FTS configuration to be used as template for the new one
     * @param string $withmap
     *            Should we copy whole map of existing FTS configuration to the new one
     * @param string $makeDefault
     *            Should this configuration be the default for locale given
     ...
     *
     * @param $all if
     *            false, returns schema qualified FTS confs
     *            
     * @return A recordset
     */
    function getFtsConfigurations($all = true) {
        ...
        if (! $all) $sql .= " AND  n.nspname='{$c_schema}'\n";
     ...
     *
     * @param $all if
     *            false, return only Parsers from the current schema
     *            
     * @return RecordSet
     */
     ...
     *
     * @param $all if
     *            false, return only Dics from the current schema
     *            
     * @returns RecordSet
     */
     ...
     *            The name of the FTS dictionary to create
     * @param boolean $isTemplate
     *            Flag whether we create usual dictionary or dictionary template
     * @param string $template
     *            The existing FTS dictionary to be used as template for the new one
     * @param string $lexize
     *            The name of the function, which does transformation of input word
     * @param string $init
     *            The name of the function, which initializes dictionary
     * @param string $option
     *            Usually, it stores various options required for the dictionary
     * @param string $comment
     *            If omitted, defaults to nothing
     *            
     * @return 0 success
     ...
     *
     * @param string $cfgname
     *            The name of the FTS configuration to alter
     * @param array $mapping
     *            Array of tokens' names
     * @param string $action
     *            What to do with the mapping: add, alter or drop
     * @param string $dictname
     *            Dictionary that will process tokens given or null in case of drop action
     *            
     * @return 0 success
     */
     ...
     *
     * @param $ftscfg The
     *            config's name that use the parser
     *            
     * @return 0 on success
     */
     ...
     *
     * @param $all True
     *            to get all languages, regardless of show_system
     * @return A recordset
     */
    function getLanguages($all = false) {
        global $conf;
        
        if ($conf ['show_system'] || $all) $where = '';
        else $where = 'WHERE lanispl';
     ...
     * @param $basetype The
     *            input data type of the aggregate
     * @param $sfunc The
     *            name of the state transition function for the aggregate
     * @param $stype The
     *            data type for the aggregate's state value
     * @param $ffunc The
     *            name of the final function for the aggregate
     * @param $initcond The
     *            initial setting for the state value
     * @param $sortop The
     *            sort operator for the aggregate
     * @param $comment Aggregate
     *            comment
     * @return 0 success
     * @return -1 error
     */
     ...
     *
     * @param $aggrname The
     *            actual name of the aggregate
     * @param $aggrtype The
     *            actual input data type of the aggregate
     * @param $newaggrname The
     *            new name of the aggregate
     * @return 0 success
     */
     ...
     * @param $aggrtype The
     *            input data type of the aggregate
     * @param $newaggrowner The
     *            new owner of the aggregate
     * @return 0 success
     */
     ...
     * @param $aggrtype The
     *            input data type of the aggregate
     * @param $newaggrschema The
     *            new schema for the aggregate
     * @return 0 success
     */
     ...
     *            actual input data type of the aggregate
     * @param $aggrowner The
     *            actual owner of the aggregate
     * @param $aggrschema The
     *            actual schema the aggregate belongs to
     * @param $aggrcomment The
     *            actual comment for the aggregate
     * @param $newaggrname The
     *            new name of the aggregate
     * @param $newaggrowner The
     *            new owner of the aggregate
     * @param $newaggrschema The
     *            new schema where the aggregate will belong to
     * @param $newaggrcomment The
     *            new comment for the aggregate
     * @return 0 success
     ...
     *            The role name to exclude from the select
     * @return All roles
     */
     ...
     * @param $rolename The
     *            name of the role that will belong to the target role
     * @param $admin (optional)
     *            Flag to grant the admin option
     * @return 0 success
     */
     ...
     *            name of the target role
     * @param $rolename The
     *            name of the role that will not belong to the target role
     * @param $admin (optional)
     *            Flag to revoke only the admin option
     * @param $type (optional)
     *            Type of revoke: RESTRICT | CASCADE
     * @return 0 success
     */
     ...
     * @param $superuser Boolean
     *            whether or not the role is a superuser
     * @param $createdb Boolean
     *            whether or not the role can create databases
     * @param $createrole Boolean
     *            whether or not the role can create other roles
     * @param $inherits Boolean
     *            whether or not the role inherits the privileges from parent roles
     * @param $login Boolean
     *            whether or not the role will be allowed to login
     * @param $connlimit Number
     *            of concurrent connections the role can make
     * @param $expiry String
     *            Format 'YYYY-MM-DD HH:MM:SS'. '' means never expire
     * @param $memberof (array)
     *            Roles to which the new role will be immediately added as a new member
     * @param $members (array)
     *            Roles which are automatically added as members of the new role
     * @param $adminmembers (array)
     *            Roles which are automatically added as admin members of the new role
     * @return 0 success
     */
    function createRole($rolename, $password, $superuser, $createdb, $createrole, $inherits, $login, $connlimit, $expiry, $memberof, $members, $adminmembers) {
        ...
        if ($connlimit != '') $sql .= " CONNECTION LIMIT {$connlimit}";
        else $sql .= ' CONNECTION LIMIT -1';
     ...
     *            Format 'YYYY-MM-DD HH:MM:SS'. '' means never expire
     * @param $memberof (array)
     *            Roles to which the role will be immediately added as a new member
     * @param $members (array)
     *            Roles which are automatically added as members of the role
     * @param $adminmembers (array)
     *            Roles which are automatically added as admin members of the role
     * @param $memberofold (array)
     *            Original roles whose the role belongs to
     * @param $membersold (array)
     *            Original roles that are members of the role
     * @param $adminmembersold (array)
     *            Original roles that are admin members of the role
     * @return 0 success
     */
     ...
     *
     * @param $rolename The
     *            name of the role to rename
     * @param $newrolename The
     ...
     * @param $createdb boolean
     *            Whether or not the user can create databases
     * @param $createuser boolean
     *            Whether or not the user can create other users
     * @param $expiry string
     *            Format 'YYYY-MM-DD HH:MM:SS'. '' means never expire
     * @param $group (array)
     *            The groups to create the user in
     * @return 0 success
     */
    ...
    function _parseACL($acl) {
        ...
        while ( $i < strlen ( $acl ) ) {
            ...
            // enter quoted bit
            $char = substr ( $acl, $i, 1 );
            if ($char == '"' && ($i == 0 || substr ( $acl, $i - 1, 1 ) != '\\')) $in_quotes = ! $in_quotes;
        ...
        foreach ( $aces as $v ) {
            ...
            for($i = 0; $i < strlen ( $chars ); $i ++) {
                ...
                if ($char == '*') $row [4] [] = $this->privmap [substr ( $chars, $i - 1, 1 )];
                elseif ($char == '/') {
                    ...
                    if (! isset ( $this->privmap [$char] )) return - 3;
                    else $row [2] [] = $this->privmap [$char];
                }
            }
     ...
     *
     * @param $object The
     *            name of the object whose privileges are to be retrieved
     * @param $type The
     *            type of the object (eg. database, schema, relation, function or language)
     * @param $table Optional,
     *            column's table if type = column
     * @return Privileges array
     ...
     *            name of the object
     * @param $public True
     *            to grant to public, false otherwise
     * @param $usernames The
     *            array of usernames to grant privs to.
     * @param $groupnames The
     *            array of group names to grant privs to.
     * @param $privileges The
     *            array of privileges to grant (eg. ('SELECT', 'ALL PRIVILEGES', etc.) )
     * @param $grantoption True
     *            if has grant option, false otherwise
     * @param $cascade True
     *            for cascade revoke, false otherwise
     * @param $table the
     *            column's table if type=column
     * @return 0 success
     * @return -1 invalid type
    ...
    function setPrivileges($mode, $type, $object, $public, $usernames, $groupnames, $privileges, $grantoption, $cascade, $table) {
        ...
        if (! is_array ( $usernames ) || ! is_array ( $groupnames ) || (! $public && sizeof ( $usernames ) == 0 && sizeof ( $groupnames ) == 0)) return - 4;
     ...
     *
     * @param $all Include
     *            all tablespaces (necessary when moving objects back to the default space)
     * @return A recordset
     */
     ...
     *
     * @param $spcname The
     *            name of the tablespace to create
     * @param $spcowner The
     *            owner of the tablespace. '' for current
     * @param $spcloc The
     *            directory in which to create the tablespace
     * @return 0 success
     */
     ...
     * @param $name The
     *            new name for the tablespace
     * @param $owner The
     *            new owner for the tablespace
     * @return 0 success
     * @return -1 transaction error
     * @return -2 owner error
     * @return -3 rename error
     ...
     *            table to vacuum
     * @param $analyze If
     *            true, also does analyze
     * @param $full If
     *            true, selects "full" vacuum
     * @param $freeze If
     *            true, selects aggressive "freezing" of tuples
     */
     ...
     * @return A recordset
     */
    function saveAutovacuum($table, $vacenabled, $vacthreshold, $vacscalefactor, $anathresold, $anascalefactor, $vaccostdelay, $vaccostlimit) {
        $f_schema = $this->_schema;
        $this->fieldClean ( $f_schema );
        $this->fieldClean ( $table );
    ...
    function getProcesses($database = null) {
        if ($database === null) $sql = "SELECT datname, usename, pid, 
    ...
    function getLocks() {
        ...
        if (! $conf ['show_system']) $where = 'AND pn.nspname NOT LIKE $$pg\_%$$';
        else $where = "AND nspname !~ '^pg_t(emp_[0-9]+|oast)$'";
    ...
    function sendSignal($pid, $signal) {
        ...
        if ($signal == 'CANCEL') $sql = "SELECT pg_catalog.pg_cancel_backend({$pid}) AS val";
        elseif ($signal == 'KILL') $sql = "SELECT pg_catalog.pg_terminate_backend({$pid}) AS val";
        else return - 1;
     ...
     * @param $obj_type One
     *            of 'TABLE' | 'COLUMN' | 'VIEW' | 'SCHEMA' | 'SEQUENCE' | 'TYPE' | 'FUNCTION' | 'AGGREGATE'
     * @param $obj_name The
     *            name of the object for which to attach a comment.
     * @param $table Name
     *            of table that $obj_name belongs to. Ignored unless $obj_type is 'TABLE' or 'COLUMN'.
     * @param $comment The
     *            comment to add.
     * @return 0 success
     */
     ...
     * character by 1.
     * In psql this is careful to take into account
     * multibyte languages, but we don't at the moment, so this function
     * is someone redundant, since it will always advance by 1
     *
     * @param
     *            &$i The current character position in the line
     * @param
     *            &$prevlen Length of previous character (ie. 1)
     * @param
     *            &$thislen Length of current character (ie. 1)
     */
    ...
    /**
     * Executes an SQL script as a series of SQL statements.
     * Returns
     ...
     *            Callback function to call with each query,
    ...
    function executeScript($name, $callback = null) {
        ...
        while ( ! feof ( $fd ) ) {
            ...
            for($i = 0; $i < $len; $this->advance_1 ( $i, $prevlen, $thislen )) {
                ...
                if ($i > 0 && substr ( $line, $i - $prevlen, 1 ) == '\\') $bslash_count ++;
                else $bslash_count = 0;
                ...
                if ($in_quote !== 0) {
                     ...
                     */
                    if (substr ( $line, $i, 1 ) == $in_quote && ($bslash_count % 2 == 0 || $in_quote == '"')) $in_quote = 0;
                }
                ...
                /* start of extended comment? */
                    ...
                    $in_xcomment ++;
                    if ($in_xcomment == 1) $this->advance_1 ( $i, $prevlen, $thislen );
                }
                
                /* in or end of extended comment? */
                else if ($in_xcomment) {
                    if (substr ( $line, $i, 2 ) == '*/' && ! -- $in_xcomment) $this->advance_1 ( $i, $prevlen, $thislen );
                }
                ...
                /* semicolon? then send query */
                    ...
                    if (strspn ( $subline, " \t\n\r" ) != strlen ( $subline )) {
                         ...
                         * line in the buffer
                         */
                        if (strlen ( $query_buf ) > 0) $query_buf .= "\n";
                        $query_buf .= $subline;
                    }
                    $query_buf .= ';';
     ...
     * @param $show An
     *            array of columns to show. Empty array means all columns.
     * @param $values An
     *            array mapping columns to values
     * @param $ops An
     *            array of the operators to use
     * @param $orderby (optional)
     *            An array of column numbers or names (one based)
    ...
    function getSelectSQL($table, $show, $values, $ops, $orderby = array ()) {
        ...
        if (sizeof ( $show ) == 0) {
            if ($this->hasObjectID ( $table )) $sql = "SELECT \"{$this->id}\", * FROM ";
            else $sql = "SELECT * FROM ";
        }
        else {
            // Add oid column automatically to results for editing purposes
            if (! in_array ( $this->id, $show ) && $this->hasObjectID ( $table )) $sql = "SELECT \"{$this->id}\", \"";
            else $sql = "SELECT \"";
    ...
    /**
     * Returns a recordset of all columns in a query.
     * Supports paging.
     *
     * @param $type Either
     *            'QUERY' if it is an SQL query, or 'TABLE' if it is a table identifier,
     *            or 'SELECT" if it's a select query
     * @param $table The
     *            base table of the query. NULL for no table.
     * @param $query The
     *            query that is being executed. NULL for no query.
     * @param $sortkey The
     *            column number to sort by, or '' or null for no sorting
     * @param $sortdir The
     *            direction in which to sort the specified column ('asc' or 'desc')
     * @param $page The
     *            page of the relation to retrieve
     * @param $page_size The
     *            number of rows per page
     * @param
     *            &$max_pages (return-by-ref) The max number of pages in the relation
     * @return A recordset on success
     * @return -1 transaction error
     ...
     *            name of a table
     * @param $key The
     *            associative array holding the key to retrieve
     * @return A recordset
     */
     ...
     *
     * @param $table The
     *            table to fetch index stats for
     * @return A recordset
     */
